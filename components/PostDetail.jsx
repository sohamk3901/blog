...
const PostDetail = ({ post }) => {
  const getContentFragment = (index, text, obj, type) => {
    ...
    switch (type) {
      case 'heading-three':
        return <h3 key={index} className="text-3xl text-white font-semibold mb-4">{modifiedText.map((item, i) => <React.Fragment key={i}>{item}</React.Fragment>)}</h3>;
      case 'paragraph':
        return <p key={index} className="mb-8 text-white text-lg">{modifiedText.map((item, i) => <React.Fragment key={i}>{item}</React.Fragment>)}</p>;
      case 'heading-four':
        return <h4 key={index} className="text-2xl text-white font-semibold mb-4">{modifiedText.map((item, i) => <React.Fragment key={i}>{item}</React.Fragment>)}</h4>;
      case 'image':
        return (
    ...
    <>
      <div className="bg-black shadow-lg rounded-lg lg:p-8 pb-12 mb-8">
        ...
        <div className="px-4 lg:px-0">
          <div className="flex items-center mb-8 w-full border-b pb-6 border-white">
